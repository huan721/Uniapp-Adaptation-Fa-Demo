...
const {
  src,
  lib
} = require('@dcloudio/uni-' + process.env.UNI_PLATFORM + '/path')
...
module.exports = (api, options) => {
  ...
  if (process.env.NODE_ENV !== 'production' || (process.env.UNI_PLATFORM !== 'h5' && process.env.UNI_PLATFORM !== 'hos')) {
    return
  }
  ...
  const uniHosAppComponentsPath = resolve('.tmp/app-components.js')
  const uniHosAppMixinsPath = resolve('.tmp/app-mixins.js')
  const uniHosSystemRoutes = resolve('.tmp/system-routes.js')

  options.transpileDependencies.push(/vue-cli-plugin-uni-optimize/)
  const depencies = '/uni-' + process.env.UNI_PLATFORM + '/'
  options.transpileDependencies.push(depencies)
    ...
    return {
      ...
      resolve: {
        alias: {
          ...
          'uni-hos-app-components': uniHosAppComponentsPath,
          'uni-hos-app-mixins': uniHosAppMixinsPath,
          'uni-hos-system-routes': uniHosSystemRoutes
        }
      },
      plugins: [
