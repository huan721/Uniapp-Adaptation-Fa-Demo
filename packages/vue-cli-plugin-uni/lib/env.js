...
const isH5 = !process.env.UNI_SUB_PLATFORM && (process.env.UNI_PLATFORM === 'h5' || process.env.UNI_PLATFORM === 'hos')
...
function initUtsPlatform () {
  ...
  if (process.env.UNI_PLATFORM === 'h5' || process.env.UNI_PLATFORM === 'hos') {
...
if (process.env.UNI_PLATFORM === 'h5' || process.env.UNI_PLATFORM === 'hos') {
  ...
  if (optimization) {
    ...
    if (
      ...
      if (Array.isArray(treeShaking.modules) && treeShaking.modules.length) {
        ...
        try {
          const modules = require('@dcloudio/uni-' + process.env.UNI_PLATFORM + '/lib/modules.json')
  ...
  const indexCssBuffer = fs.readFileSync(require.resolve('@dcloudio/uni-' + process.env.UNI_PLATFORM + '/dist/index.css'))
  process.env.VUE_APP_INDEX_CSS_HASH = loaderUtils.getHashDigest(indexCssBuffer, 'md5', 'hex', 8)
  let indexDarkCssBuffer = ''
  try {
    indexDarkCssBuffer = fs.readFileSync(require.resolve('@dcloudio/uni-' + process.env.UNI_PLATFORM + '/dist/index.dark.css'))
...
if (
  ...
  process.env.UNI_PLATFORM === 'h5' || process.env.UNI_PLATFORM === 'hos'
) { // 自定义组件模式或 h5 平台
...
} else if (process.env.UNI_PLATFORM !== 'h5' && process.env.UNI_PLATFORM !== 'hos' && process.env.UNI_PLATFORM !== 'quickapp-native' ) {
  try {
