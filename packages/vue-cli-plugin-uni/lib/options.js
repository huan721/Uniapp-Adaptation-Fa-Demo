...
module.exports = function initOptions (options) {
  ...
  if (process.env.UNI_PLATFORM === 'h5' || process.env.UNI_PLATFORM === 'hos') { // h5 dev 用到了这两个，需要 babel
  ...
  if (process.env.UNI_PLATFORM === 'h5' || process.env.UNI_PLATFORM === 'hos' || process.env.UNI_USING_V3) {
